...
function toNumber(value, def) {
  return value == null ? def : Number(value) || def;
}
...
function isPowerOfTwo(num) {
  ...
  return num / 2 === Math.round(num / 2) ? isPowerOfTwo(num / 2) : false;
}
...
function getCountOfOddNumbers(number) {
  let res = 0;
  for (let i = 1; i <= Math.abs(number); i += 1) if (i % 2) res += 1;
  return res;
}
